...
class TestGrab < Test::Unit::TestCase 
  ...
  test 'BioPieces::Pipeline::Grab with keys_only and values_only options raises' do
    assert_raise(@e) { @p.grab(select: 'foo', keys_only: true, values_only: true) }
  end
  ...
  test 'BioPieces::Pipeline::Grab with keys and keys_only or valuess_only raises' do
    assert_raise(@e) { @p.grab(keys: :FOO, keys_only: true) }
    assert_raise(@e) { @p.grab(keys: :FOO, values_only: true) }
  end
  ...
  test 'BioPieces::Pipeline::Grab with select and value hit return correctly' do
    ...
    expected = %Q({:SEQ_NAME=>"test1", :SEQ=>"atcg", :SEQ_LEN=>4}\n)
    assert_equal(expected, collect_result)
  end
...
#
#    expected = <<-EXP.gsub(/^\s+\|/, '')
#      |{:SEQ_NAME=>'test', :SEQ=>'atcg', :SEQ_LEN=>4}
#      |{:SEQ_NAME=>"test2", :SEQ=>"DSEQM", :SEQ_LEN=>5}
#    EXP
#
#    assert_equal(expected, collect_result)
...
#
#    expected = '{:SEQ_NAME=>'test', :SEQ=>'atcg', :SEQ_LEN=>4}'
#    assert_equal(expected, collect_result)
#  end
#
